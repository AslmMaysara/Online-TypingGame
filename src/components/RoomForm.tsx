...
interface RoomFormProps {
  onCreate: (data: { name: string; duration: number; lang: 'ar' | 'en' }) => void;
}
...
export function RoomForm({ onCreate }: RoomFormProps) {
  ...
  return (
    <form dir="rtl" onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block mb-2 text-sm font-medium">لغة النص</label>
        <div className="flex gap-2">
          <button
            type="button"
            className={`px-4 py-2 rounded-lg font-medium transition-all ${lang === 'ar' ? 'bg-yellow-500 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
            onClick={() => setLang('ar')}
          >
            العربية
          </button>
          <button
            type="button"
            className={`px-4 py-2 rounded-lg font-medium transition-all ${lang === 'en' ? 'bg-yellow-500 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
            onClick={() => setLang('en')}
          >
            English
          </button>
