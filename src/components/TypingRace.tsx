...
export function TypingRace({
  ...
  // Determine direction based on room.lang
  const direction = room.lang === 'en' ? 'ltr' : 'rtl';
  const textAlign = room.lang === 'en' ? 'left' : 'right';
  ...
  useEffect(() => {
    if (finished && !locked) {
      ...
      if (players.length === 1 && isHost && room.status === "in_progress") {
        finishGame({ roomId: room._id });
      }
    }
  }, [finished, locked, finishPlayer, player._id, players.length, isHost, room.status, room._id, finishGame]);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4">
        <div
          className="p-4 overflow-x-hidden text-base leading-relaxed text-white bg-gray-900 select-none sm:p-6 sm:text-lg rounded-xl"
          dir={direction}
          style={{ textAlign }}
        >
          {words.map((word, idx) => (
        ...
        <div className="relative">
          ...
          {direction === 'rtl' ? (
            ...
            <div className="absolute text-2xl -translate-y-1/2 right-4 top-1/2">
              {finished ? "🏁" : "⌨️"}
            </div>
          )}
        </div>
